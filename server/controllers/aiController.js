...
export const generateImage = async (req, res) => {
  try {
    ...
    if (plan !== 'premium' ) {   //&& free_usage >= 50
      return res.json({
        success: false,
        message: "Upgrade plan to premium to continue."
      });
    }
...
export const removeImageBackground = async (req, res) => {
  try {
    ...
    (${userId}, 'Remove background from the image',
    ${secure_url} , 'image' )`;
...
export const removeImageObject = async (req, res) => {
    try {
        ...
        await sql`INSERT INTO creations (user_id, prompt, content, type) 
        VALUES (${userId}, ${`Remove ${object} from the image`},
        ${imageUrl} , 'image' )`;
...
export const resumeReview = async (req, res) => {
    try {
        ...
        if(resume.size > 5*1024*1024){
            return res.json({
               success:false,
               message:'Resume file size is more than allowed size(5mb).'
            });
        }      
        ...
        the constructive feedback on its strengths, weakness and 
